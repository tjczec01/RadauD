...
class DF:
       ...
       def forward_subd(self, L, b):
           ...
           for i in range(len(b)):
               ...
               for j in range(i):
                     ...
                     y[i]= v3
               y[i] = self.dmake(self.dmake(y[i])/self.dmake(L[i][i]))
       ...
       def backward_subd(self, U, y):
           ...
           for i in range(len(x)-1, 0, -1):
               val_i = self.dmake((self.dmake(y[i-1]) - self.dmake(self.dotd(U[i-1][i:], x[i:]))) / self.dmake(U[i-1][i-1]))
               x[i-1] = self.dmake(val_i)
       ...
       def forward_sub(self, L, b):
           ...
           for i in range(len(b)):
               ...
               try:
                   try:
                       y[i] = self.dmake(y[i][0])/self.dmake(L[i][i][0])
                   except:
                       try:
                           y[i] = self.dmake(y[i][0])/self.dmake(L[i][i])
                       except:
                           pass
               except:
       ...
       def backward_sub(self, U, y):
           ...
           for i in range(len(x), 0, -1):
               try:
                        val_i = (complex(y[i-1].real, y[i-1].imag) - np.dot(U[i-1][i:], XX[i:])) / complex(U[i-1][i-1].real, U[i-1][i-1].imag)
               except:
                        val_i = self.dmake(y[i-1] - np.dot(U[i-1][i:], x[i:])) / self.dmake(U[i-1][i-1][0])
               x[i-1] = val_i
